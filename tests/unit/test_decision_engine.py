...
def test_low_confidence_overrides_high_rule_score_to_manual_review() -> None:
    output = compute_decision(
        result({"credit_score": 800}, FailureType.STALE_DATA),
        result({"income_stability": 0.8}, FailureType.PARTIAL_DATA),
        result({"gst_compliant": False}, FailureType.PAN_MISMATCH, fallback_used=True),
        USER_DATA,
    )
...
def test_confidence_exactly_at_threshold_is_not_overridden() -> None:
    output = compute_decision(
        result({"credit_score": 700}, FailureType.STALE_DATA),
        result({"income_stability": 0.7}, FailureType.PAN_MISMATCH),
        result({"gst_compliant": True}),
        USER_DATA,
...
def test_bad_credit_declines_when_confidence_is_high_enough() -> None:
    output = compute_decision(
        result({"credit_score": 550}),
        result({"income_stability": 0.9}),
        result({"gst_compliant": True}),
        USER_DATA,
    )
...
def test_partial_bank_data_uses_neutral_stability() -> None:
    ...
    assert output.decision == Decision.APPROVE
    assert "income_stability (partial) = 0.5" in output.factors
...
def test_missing_credit_without_fallback_uses_default_source() -> None:
    ...
    assert "credit_score (live) = 600" in output.factors
    assert output.confidence == 1.0
...
def test_format_error_bank_data_uses_default_stability() -> None:
    ...
    assert "income_stability (default) = 0.5" in output.factors
...
def test_gst_no_record_defaults_to_non_compliant() -> None:
    ...
    assert "gst_compliant (fallback) = False" in output.factors
